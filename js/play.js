...
function checkPair($current) {
    ...
    if (isFirstPlay) {
        ...
        let isEqual = $previous.dataset.number == $current.dataset.number;
        ...
        if (isEqual) {
            ...
            if (document.querySelectorAll(".matched").length == size * size) {
                ...
                if (players > 1) {
                    ...
                    for (let index = 0; index < Number(localStorage.getItem("players")); index++) {
                    ...
                    if (hasDuplicateOf($max, $scores)) document.querySelector("#who_won").innerText = `It's a tie!`;
                    else {
                        document.querySelector("#who_won").innerText = `Player ${$scores.indexOf($max) + 1} wins!`;
                    }

                    //indicate winner / winners
                    document.querySelectorAll(".tile").forEach((element, index) => {
                        if ($scores[index] == $max) element.classList.add("winner");
                    });
                } else {
                    stopTimer()
                    cards = `<div class="flex tile justify-between"><div>Time Elapsed</div><div id="time_elapsed"></div></div>
            <div class="flex tile justify-between"><div>Moves Taken</div><div id="moves_taken"></div></div>`;
                    document.querySelector("#result_cards").innerHTML = cards;

                    document.querySelector("#who_won").innerText = `You did it!`;

                    document.querySelector(".modal").style.display = "block";
                    document.querySelector("#time_elapsed").innerText = document.querySelector("#timer").innerText;
                    document.querySelector("#moves_taken").innerText = document.querySelector("#moves").innerText + " Moves";
                    document.querySelector("#subtext").innerText = "Game over! Here's how you got on...";
                }
            }
        } else {
